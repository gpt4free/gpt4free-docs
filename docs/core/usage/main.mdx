...
<ParamField path="proxy" type="string">
  The proxy to use for the API request. Defaults to `None`.
</ParamField>

<ParamField path="response_format" type="dict">
  The format of the response from the API. Defaults to `None`.
</ParamField>

<ParamField path="max_tokens" type="integer">
  The maximum number of tokens to generate in the response. Defaults to `None`.
</ParamField>

<ParamField path="stop" type="list[string] | string">
  A list of strings or a single string to stop the generation at. Defaults to `None`.
</ParamField>
