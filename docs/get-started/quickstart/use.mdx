...
`g4f.client.Client` class Params
...
`client.chat.completions.create` basic Params (For more advanced usage and examples, refer to the [Full Documentation](/docs)):
...
<ParamField path="provider" type="ProviderType" default="Auto">
  The provider to use for the chat completion
</ParamField>

<ParamField path="stream" type="bool" default="False">
  Whether to stream the response, `create` will return a generator.
</ParamField>
